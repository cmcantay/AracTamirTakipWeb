...
namespace AracTamirTakip.Web.Controllers
{
    public class IsemriController : Controller
    {
        ...
        public ActionResult KaydetveKapat(int IsemriId,string OdemeSekli,string Aciklama,decimal AlinanUcret)
        {
            ...
            if (isemri.Kapali)
            {
                TempData["No"] = "Bu İş Emri Zaten Kapalı!";
                return RedirectToAction("AcikIsemirleri");
            }
        ...
        {
            ViewBag.Islemler = rpIslem.Get(x => x.IsemriId == isemriId).OrderByDescending(x=>x.IsemriId).ToList();
